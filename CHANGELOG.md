...
- **Bridge fix + test layer**: fixed a signedness bug in the CIDR allowlist — networks with the first octet ≥ 128 (e.g. `172.31.50.0/24`) never matched and every request got 403. Caught by the new unit tests: pure helpers (auth, CIDR, rate limit, size caps, command templates) now live in `scripts/bridge-lib.mjs` with `node --test` coverage gating CI before the compose stack builds. CI also gained wrong-token 401, rate-limit 429, and bridge-survives-restart checks.
